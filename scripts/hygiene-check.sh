#!/bin/zsh
# Public-repo hygiene: fail if tracked files contain machine paths, personal identifiers or credentials.
set -u
...
hits=$(git ls-files | xargs grep -nIE "$pattern" 2>/dev/null | grep -vE "$allow")
if [ -n "$hits" ]; then
