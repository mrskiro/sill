...
///
/// A phone never advertises, so dialling one is a browse that can never match (and a local-network
/// prompt for nothing). Peers say what they are in `hello`, so phones are left out. A peer that has
/// not said hello since the field existed stays a candidate: the first hello settles it.
public enum SyncRole {
    ...
    public static func dialTargets(myDeviceID: DeviceID, peers: [Peer]) -> [Peer] {
        peers.filter { $0.kind != .phone && shouldDial(myDeviceID: myDeviceID, peerID: $0.id) }
    }
}
