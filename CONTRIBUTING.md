...
deliberately left out. `metadata/` holds the App Store copy for the iPhone app, one
folder per version. `CLAUDE.md` is a short working guide with the traps this codebase
has already hit.
...
submits it for App Review with the copy in `metadata/version/X.Y.Z/`; an approved build
goes on sale by itself. Each job needs its own secrets, so a fork without them cannot
cut a release.
