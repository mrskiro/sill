...
@Suite struct LoopbackSyncTests {
    ...
    private func startListener(
        _ mac: Side, server: SyncServer
    ) async throws -> (task: Task<Void, any Error>, port: UInt16) {
        let (ports, continuation) = AsyncStream<UInt16>.makeStream()
        let task = Task {
            try await SillListener(identity: mac.identity, server: server, advertise: false).run {
    ...
    @Test func pairsAndSyncsOverMutualTLS() async throws {
        let mac = try Side("Mac")
        let phone = try Side("iPhone")
        defer {
            try? mac.identityStore.delete()
            try? phone.identityStore.delete()
        }
        ...
        let clientTask = Task {
            try await SillConnector.connect(
                to: endpoint, identity: phone.identity, client: client,
    ...
    @Test func unknownCertificateNeverGetsASession() async throws {
        let mac = try Side("Mac")
        let stranger = try Side("Stranger")
        defer {
            try? mac.identityStore.delete()
            try? stranger.identityStore.delete()
        }
        let server = SyncServer(store: mac.store)
        let client = SyncClient(store: stranger.store)
        let listener = try await startListener(mac, server: server)
        let endpoint = NWEndpoint.hostPort(host: "127.0.0.1", port: NWEndpoint.Port(rawValue: listener.port)!)
        ...
        let clientTask = Task {
            try await SillConnector.connect(
                to: endpoint, identity: stranger.identity, client: client,
