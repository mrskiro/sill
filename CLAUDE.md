...
- 同じ基準を PR 本文・コミットメッセージ・Issue にも適用する。`make hygiene` はファイルしか見ないので、ここは手で守るしかない。とくに claude.ai のアーティファクト / セッション URL は他人が開けないため載せない（必要な内容はリポジトリ内に書く）。ローカルパスやワークツリー名も同様。
