...
- リリースはメンテナだけが行う。`metadata/version/X.Y.Z/` に `metadata/app-info/` と同じ言語のファイル（いまは `en-US.json` と `ja.json`。What's New と説明文）を置き、PR で main に入れてから `vX.Y.Z` タグを push する。あとは `.github/workflows/release.yml` が行う。Mac は Developer ID 署名と公証を済ませた dmg を Release に添付する。iOS は `asc publish appstore` でアップロードから審査提出までを行い、承認されると自動で公開される。
